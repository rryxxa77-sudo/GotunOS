...
void InstallWorker::log(const QString &msg) {
    emit logLine(msg);
    if (m_logFile.isOpen()) {
...
// Single-quote a value for safe use inside the generated bash scripts.
static QString shq(const QString &value) {
    QString v = value;
    v.replace("'", "'\\''");
    return "'" + v + "'";
}
...
bool InstallWorker::sh(const QStringList &lines, const QString &tag) {
    ...
    const bool ok = cmd("/bin/bash", {path});
    QFile::remove(path);
    return ok;
}

// Passwords never touch the disk: they go straight into chpasswd's stdin.
bool InstallWorker::setPassword(const QString &user, const QString &password) {
    QProcess p;
    p.start("arch-chroot", {"/mnt", "chpasswd"});
    if (!p.waitForStarted(5000)) { log("ERROR: cannot start chpasswd"); return false; }
    p.write((user + ":" + password + "\n").toUtf8());
    p.closeWriteChannel();
    p.waitForFinished(30000);
    if (p.exitStatus() != QProcess::NormalExit || p.exitCode() != 0) {
        log("ERROR: could not set the password for " + user);
        return false;
    }
...
void InstallWorker::run() {
    // Write log file
    m_logFile.setFileName("/var/log/gotunos-install.log");
    m_logFile.open(QIODevice::WriteOnly | QIODevice::Text);

    auto runStep = [&](bool ok, const char *name) -> bool {
        if (!ok) {
            log(QString("FATAL: step '%1' failed").arg(name));
            m_logFile.close();
            emit installFinished(false, QString("Step failed: %1").arg(name));
            return false;
        }
        return true;
    };
    ...
    if (!runStep(stepSwapFile(),        "Swap"))        return;
...
bool InstallWorker::stepHostname() {
    ...
    return sh({
        QString("echo %1 > /mnt/etc/hostname").arg(shq(m_cfg.hostname)),
...
bool InstallWorker::stepUsers() {
    ...
    if (!sh(s, "users")) return false;

    if (!setPassword("root", m_cfg.rootPassword)) return false;
    for (const auto &u : m_cfg.users)
        if (!setPassword(u.username, u.password)) return false;
...
bool InstallWorker::stepBootloader() {
    ...
    if (m_cfg.bootloader == "systemd-boot") {
        ...
        // Zen entry
        // Unquoted heredoc marker: the root UUID is a command substitution.
...
bool InstallWorker::stepDNS() {
    ...
    } else if (m_cfg.dnsProvider == "custom") {
        dns1 = m_cfg.dnsIp1; dns2 = m_cfg.dnsIp2; dnsHost = m_cfg.dnsHost;
        if (dns1.isEmpty()) { log("No custom DNS address given, skipping DNS setup"); return true; }
        if (dns2.isEmpty()) dns2 = dns1;
    } else {
        return true;
    }

    // Without a TLS hostname the resolver cannot validate the certificate, so
    // fall back to plain DNS instead of silently failing every lookup.
    const QString dnsLine = dnsHost.isEmpty()
        ? QString("DNS=%1 %2\\nDNSOverTLS=no\\n").arg(dns1, dns2)
        : QString("DNS=%1#%3 %2#%3\\nDNSOverTLS=yes\\n").arg(dns1, dns2, dnsHost);

    return sh({
        "arch-chroot /mnt systemctl enable systemd-resolved",
        "mkdir -p /mnt/etc/systemd/resolved.conf.d",
        QString("printf '[Resolve]\\n%1DNSSEC=yes\\n' "
                "> /mnt/etc/systemd/resolved.conf.d/gotunos.conf").arg(dnsLine),
...
bool InstallWorker::stepSwapFile() {
    if (!m_cfg.swapEnabled) return true;
    step("Creating swap file", 65);

    const int sizeGiB = qBound(1, m_cfg.swapSizeGiB, 64);
    QStringList s;
    // btrfs needs a no-COW file created with truncate/chattr, everything else
    // is happy with fallocate.
    if (m_cfg.filesystem == "btrfs") {
        s << "btrfs filesystem mkswapfile --size " + QString::number(sizeGiB) + "g /mnt/swapfile";
    } else {
        s << QString("fallocate -l %1G /mnt/swapfile").arg(sizeGiB);
        s << "chmod 600 /mnt/swapfile";
        s << "mkswap /mnt/swapfile";
    }
    s << "echo '/swapfile none swap defaults 0 0' >> /mnt/etc/fstab";
    return sh(s, "swapfile");
}
...
bool InstallWorker::stepSecondaryDrives() {
    ...
    for (const auto &drv : m_cfg.secondaryDrives) {
        ...
        s << QString("mkdir -p %1").arg(shq(mntAbs));
        ...
        if (drv.fsType == "ntfs" || drv.fsType == "vfat" || drv.fsType == "exfat") {
            s << QString("mount -o uid=1000,gid=1000,rw,umask=0000,dmask=0000,fmask=0000 %1 %2 || true")
                     .arg(drv.partition, shq(mntAbs));
            s << QString("chmod 777 %1 2>/dev/null || true").arg(shq(mntAbs));
            // Fstab entry with nofail + permissions Steam/Proton is happy with
            // Double quotes: the UUID comes from a command substitution.
            ...
            s << QString("mount %1 %2 || true").arg(drv.partition, shq(mntAbs));
            s << QString("chmod 777 %1").arg(shq(mntAbs));
            s << QString("chown -R 1000:1000 %1").arg(shq(mntAbs));
        ...
        if (!drv.symlinkName.isEmpty()) {
            for (const auto &u : m_cfg.users) {
                const QString link = QString("/mnt/home/%1/%2").arg(u.username, drv.symlinkName);
                s << QString("ln -sf %1 %2").arg(shq("/" + mntRel), shq(link));
                s << QString("chown -h 1000:1000 %1").arg(shq(link));
            }
        }
    }
