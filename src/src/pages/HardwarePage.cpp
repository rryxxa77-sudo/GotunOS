...
HardwarePage::HardwarePage(InstallConfig *cfg, QWidget *parent) : BasePage(cfg, parent) {
    ...
    m_detected = new QLabel("Autodetecting hardware...");
...
void HardwarePage::onShow() { autodetect(); }
...
void HardwarePage::autodetect() {
    ...
    if (pci.contains("nvidia")) {
        // Default to open-source drivers for modern NVIDIA, fallback available
        m_gpuBox->setCurrentIndex(1);
