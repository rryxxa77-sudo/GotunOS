...
void UsersPage::addUserRow() {
    ...
    if (idx == 0) {
        ...
        connect(row.remove, &QPushButton::clicked, this, [this, frame, idx]() {
            frame->deleteLater();
            if (idx < m_rows.size()) m_rows.removeAt(idx);
        });
...
void UsersPage::removeUserRow(int idx) {
    Q_UNUSED(idx)
}
...
bool UsersPage::isValid() {
    ...
    for (int i = 0; i < m_rows.size(); ++i) {
        ...
        if (r.username->text().trimmed().isEmpty()) continue; // Skip empty rows
        
        if (r.password->text() != r.confirm->text()) {
            QMessageBox::warning(this, I18n::tr("users"), I18n::language() == "tr" ? "Kullanıcı şifreleri eşleşmiyor!" : "User passwords do not match!");
            r.password->setFocus();
            return false;
        }
    }

    // Validate Root Password (if not synced)
    if (!m_samePassChk->isChecked()) {
        if (m_rootPass->text() != m_rootConfirm->text()) {
            QMessageBox::warning(this, I18n::tr("users"), I18n::language() == "tr" ? "Root şifreleri eşleşmiyor!" : "Root passwords do not match!");
            m_rootPass->setFocus();
            return false;
        }
    }
