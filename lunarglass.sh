...
BACKUP_DIR="$HOME/Lunar-Glass-backup-$(date +%Y%m%d_%H%M%S)"
DISTRO="unknown"
...
section() {
    printf "\n\033[1;37m╔═══════════════════════════════════════════════════════╗\033[0m\n"
    printf "\033[1;37m║\033[0m  %-51s \033[1;37m║\033[0m\n" "$1"
...
fetch_repo() {
    log "syncing Lunar Glass repository…"
    if [ ! -d "$REPO_DIR/.git" ]; then
        if git clone https://github.com/MurderFromMars/Lunar-Glass "$REPO_DIR" 2>&1 | grep -v -E "^(remote:|Receiving|Resolving|Counting)" | grep -v "^$" || false; then
...
deploy_color_scheme() {
    log "installing CosmicMonochromia color scheme…"
    mkdir -p "$HOME/.local/share/color-schemes"
    if [ -f "$REPO_DIR/CosmicMonochromia.colors" ]; then
        cp "$REPO_DIR/CosmicMonochromia.colors" "$HOME/.local/share/color-schemes/"
        ok "colors → CosmicMonochromia"
    else
        warn "CosmicMonochromia.colors missing"
    fi
}
...
deploy_wallpapers() {
    ...
    if [ -f "$REPO_DIR/lunarglass.png" ]; then
        sudo cp "$REPO_DIR/lunarglass.png" /usr/local/share/icons/
        ok "wallpaper → lunarglass.png → /usr/local/share/icons"
    else
        warn "missing → lunarglass.png"
    fi

    if [ -f "$REPO_DIR/dark2.png" ]; then
        sudo cp "$REPO_DIR/dark2.png" /usr/local/share/icons/
        ok "icon → dark2.png → /usr/local/share/icons"
    else
        warn "missing → dark2.png"
    fi

    mkdir -p "$HOME/Pictures"
    if [ -f "$REPO_DIR/dark.png" ]; then
        cp "$REPO_DIR/dark.png" "$HOME/Pictures/"
        ok "wallpaper → dark.png → ~/Pictures"
    else
        warn "missing → dark.png"
    fi
}
...
apply_kde_theme_settings() {
    log "activating Lunar Glass theme parameters…"

    if command -v plasma-apply-colorscheme >/dev/null 2>&1; then
        plasma-apply-colorscheme CosmicMonochromia 2>/dev/null || true
        ok "color scheme activated → CosmicMonochromia"
    else
        warn "plasma-apply-colorscheme not found"
    fi

    if command -v kwriteconfig6 >/dev/null 2>&1; then
        # Set custom accent color (#8cbac5)
        kwriteconfig6 --file kdeglobals --group General --key AccentColor "140,186,197"
        kwriteconfig6 --file kdeglobals --group General --key ColorSchemeHash ""
        ok "accent color → #8cbac5"
...
main() {
    printf "\n\033[1;37m┌───────────────────────────────────────────────────────┐\n"
    printf   "│    LUNAR GLASS DYNAMIC TILING THEME BY MURDERFROMMARS   │\n"
    printf   "└───────────────────────────────────────────────────────┘\033[0m\n\n"
    ...
    printf "\n\033[1;37m╔═══════════════════════════════════════════════════════╗\033[0m\n"
    printf "\033[1;37m║\033[0m  \033[1;32mLUNAR GLASS DEPLOYMENT COMPLETE\033[0m                   \033[1;37m║\033[0m\n"
    printf "\033[1;37m╚═══════════════════════════════════════════════════════╝\033[0m\n\n"
