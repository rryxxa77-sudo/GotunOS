...
REPO_DIR="$HOME/Nova-Choom"
BACKUP_DIR="$HOME/NovaChoom-backup-$(date +%Y%m%d_%H%M%S)"
DISTRO="unknown"
WALLPAPER_PATH="/usr/local/share/icons/cyberpunk.jpg"
...
fetch_repo() {
    log "syncing Nova-Choom repository…"
    if [ ! -d "$REPO_DIR/.git" ]; then
        if git clone https://github.com/MurderFromMars/Nova-Choom "$REPO_DIR" 2>&1 | grep -v -E "^(remote:|Receiving|Resolving|Counting)" | grep -v "^$" || false; then
...
deploy_color_scheme() {
    log "installing NovaChoom color scheme…"
    mkdir -p "$HOME/.local/share/color-schemes"
    if [ -f "$REPO_DIR/NovaChoom.colors" ]; then
        cp "$REPO_DIR/NovaChoom.colors" "$HOME/.local/share/color-schemes/"
        ok "colors → NovaChoom"
    else
        warn "NovaChoom.colors missing"
    fi
}
...
deploy_wallpapers() {
    ...
    if [ -f "$REPO_DIR/cyberpunk.jpg" ]; then
        sudo cp "$REPO_DIR/cyberpunk.jpg" /usr/local/share/icons/
        ok "wallpaper → cyberpunk.jpg → /usr/local/share/icons"
    else
        warn "missing → cyberpunk.jpg"
    fi

    if [ -f "$REPO_DIR/novachoom2.png" ]; then
        sudo cp "$REPO_DIR/novachoom2.png" /usr/local/share/icons/
        ok "icon → novachoom2.png → /usr/local/share/icons"
    else
        warn "missing → novachoom2.png"
    fi

    mkdir -p "$HOME/Pictures"
    if [ -f "$REPO_DIR/novachoom.png" ]; then
        cp "$REPO_DIR/novachoom.png" "$HOME/Pictures/"
        ok "wallpaper → novachoom.png → ~/Pictures"
    else
        warn "missing → novachoom.png"
    fi
}
...
apply_kde_theme_settings() {
    ...
    if command -v plasma-apply-colorscheme >/dev/null 2>&1; then
        plasma-apply-colorscheme NovaChoom 2>/dev/null || true
        ok "color scheme activated → NovaChoom"
    else
        warn "plasma-apply-colorscheme not found"
    fi

    if command -v kwriteconfig6 >/dev/null 2>&1; then
        # Set custom accent color (#fb1010)
        kwriteconfig6 --file kdeglobals --group General --key AccentColor "251,16,16"
        kwriteconfig6 --file kdeglobals --group General --key ColorSchemeHash ""
        ok "accent color → #fb1010"
...
main() {
    printf "\n\033[1;35m┌───────────────────────────────────────────────────────┐\n"
    printf   "│  NOVA-CHOOM DYNAMIC TILING THEME BY MURDERFROMMARS  │\n"
    printf   "└───────────────────────────────────────────────────────┘\033[0m\n\n"
    ...
    printf "\n\033[1;35m╔═══════════════════════════════════════════════════════╗\033[0m\n"
    printf "\033[1;35m║\033[0m  \033[1;32mNOVA-CHOOM DEPLOYMENT COMPLETE\033[0m                   \033[1;35m║\033[0m\n"
    printf "\033[1;35m╚═══════════════════════════════════════════════════════╝\033[0m\n\n"
